...
function parse(input) {
  ...
  function skip_op(op) {
    ...
    else input.croak('Expection operator: "' + op + '"');
  }
  ...
  function parse_lambda() {
    return {
      type: 'lambda',
      name: input.peek().type === 'var' ? input.next().value : null, // named function
...
function TokenStream(input) {
  let current = null;
  let keywords = ' let if then else lambda λ true false js:raw ';
  return { next, peek, eof, croak: input.croak };
  
  function is_keyword(ch) {
...
//     var jsc = make_js(exp);
//     console.error(u2.parse(jsc).print_to_string({
//         beautify: true,
//         indent_level: 2
//     }));
//     // console.error(jsc);
...
if (typeof process != 'undefined') (function(){
  ...
  var print = function(k) {
    process.stdout.write([].slice.call(arguments, 1).join(' '));
    k(false);
  };
